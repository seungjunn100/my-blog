...
| 종류 | 설명 | 대표 서비스 |
|------|------|-------------|
| 공유 호스팅 | 하나의 물리 서버를 여러 사용자가 나눠 씀 | Cafe24, 닷홈 |
| 전용 서버 | 서버 전체를 단독으로 사용 | IDC 직접 계약 |
| 클라우드 호스팅 | 트래픽 상황에 따라 서버 성능을 자동으로 늘리고 줄임 | AWS, GCP, Azure |
| 정적 호스팅 | HTML, CSS, JS 같은 정적 파일만 서빙 | Vercel, Netlify, Github Pages |

> IDC (Internet Data Center)
> > 네트워크 장비 등을 모아두고 인터넷 서비스를 안정적으로 운영하기 위한 시설이다.
...
| 레코드 | 설명 | 예시 |
|------|------|-------------|
| A | 도메인을 IPv4 주소에 연결 | `example.com` → `93.184.216.34` |
| AAAA | 도메인을 IPv6 주소에 연결 | `example.com` → `2606:2800:220:1:248:1893:25c8:1946` |
| CNAME | 도메인을 IP 주소가 아닌 또 다른 도메인 이름(별칭)으로 연결 | `www.junn.dev` → `junn.dev` |
| MX | 해당 도메인으로 오는 이메일을 수신할 메일 서버를 지정 | `junn.dev` → `aspmx.l.google.com` |
| TXT | 도메인 소유권 인증 및 이메일 보안 설정(SPF/DKIM) 등 외부 서비스와의 신뢰 구축을 위한 메타데이터를 기록 | `google-site-verification=AbCdEfG12345...` |
