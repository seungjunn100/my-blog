...
> 초기 진입 시에는 prerender된 HTML의 메타데이터가 적용되며, 클라이언트 라우팅 시에는 현재 경로를 기준으로 `<head>`를 다시 동기화해 페이지별 메타데이터를 유지하도록 구성했습니다.

<br />

## 🚨 트러블슈팅

### 메타데이터가 HTML에 반영되지 않는 문제
  
#### 문제 상황

페이지별 메타데이터를 적용하기 위해 처음에는 `react-helmet-async`를 사용해 클라이언트에서 `<head>`를 관리했다. 라우트에 따라 `<title>`과 `<meta>`를 동적으로 변경하는 방식이었다.

개발자 도구에서는 메타 태그가 정상적으로 적용된 것처럼 보였지만, Ctrl + U로 확인한 원본 HTML에는 반영되지 않는 문제가 발생했다.

#### 원인

`react-helmet-async`는 브라우저에서 React가 렌더링된 이후에 `<head>`를 수정하는 방식이다.
...
또한 클라이언트 라우팅 환경에서는 초기 HTML만으로는 페이지 전환 시 메타데이터가 갱신되지 않는 문제가 있었기 때문에, 현재 경로를 기준으로 `<head>`를 다시 동기화하는 로직을 추가해 SPA 이동 중에도 title, description, OG 태그가 올바르게 반영되도록 보완했습니다.

<br />

### gray-matter 사용 시 Buffer 에러
...
└─ src/                                   # 실제 앱 코드가 들어있는 폴더
   ...
   │  ├─ common/                          # 공통 동작/유틸 성격 컴포넌트 폴더
   │  │  └─ RouteMetaSync.tsx             # 클라이언트 라우팅 시 head를 동기화하는 컴포넌트
